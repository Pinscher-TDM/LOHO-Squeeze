...
void setupWiFi()
{
    ...
    if (settings.ssid.length() > 0) {
        WiFi.mode(WIFI_STA);
        // Do NOT call WiFi.setHostname again here
...
static void handleApiInfo()
{
    ...
    if (WiFi.getMode() == WIFI_STA && WiFi.status() == WL_CONNECTED) {
        ...
        mode = (WiFi.status() == WL_CONNECTED) ? "sta" : "ap";
    } else if (apMode) {
        mode = "ap";
    }
    ...
    json += "\"address\":\"http://" + hostname + ".local\",";
    json += "\"mac\":\"" + deviceMac + "\",";
...
void initWebServerRoutes()
{
    ...
    // CORS preflight
    server.on("/api/info", HTTP_OPTIONS, handleCorsOptions);
    server.on("/api/state", HTTP_OPTIONS, handleCorsOptions);
    server.on("/api/settings", HTTP_OPTIONS, handleCorsOptions);
    server.on("/api/control", HTTP_OPTIONS, handleCorsOptions);
    server.on("/api/releases", HTTP_OPTIONS, handleCorsOptions);
    server.on("/api/ota", HTTP_OPTIONS, handleCorsOptions);
    ...
    server.on("/api/control", HTTP_GET, []() {
        ...
        if (server.hasArg("pwm")) {
            currentPWM = constrain(server.arg("pwm").toInt(),
    ...
    server.on("/save", HTTP_POST, []() {
        ...
        if (server.hasArg("minB")) {
            settings.minBrightness = server.arg("minB").toInt();
        }
        if (server.hasArg("maxB")) {
            settings.maxBrightness = server.arg("maxB").toInt();
        }
        if (server.hasArg("speed")) {
            settings.dimSpeed = server.arg("speed").toInt();
        }
        if (server.hasArg("partyMode")) {
            settings.partyMode = server.arg("partyMode") == "on";
        }
        saveSettings();
        addCorsHeaders();
        server.sendHeader("Connection", "close");
    ...
    server.on("/api/releases", HTTP_GET, []() {
        ...
        for (size_t i = 0; i < cachedReleases.size(); i++) {
            ...
            json += "\"hasFirmware\":" + String(r.assetAppUrl.length() > 0 ? "true" : "false") + ",";
            json += "\"hasFs\":" + String(r.assetFsUrl.length() > 0 ? "true" : "false") + ",";
            json += "\"size\":" + String((unsigned long)r.assetAppSize);
            json += "}";
        }
        json += "]";
    ...
    server.on("/api/ota", HTTP_POST, []() {
        ...
        GitHubRelease release = cachedReleases[idx];
    ...
    server.on("/canonical.html", HTTP_GET, sendCaptivePortalRedirect);
